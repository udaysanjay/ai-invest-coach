  ...
  const handleSendMessage = async (customMessage?: string) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('chat-gemini', {
        body: {
          messages: [...messages, {
            type: 'user',
            content: currentMessage
        ...
        {!showChat ? <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 bg-gray-50 p-3 rounded-lg">
              {messages.map((message, index) => <div key={index} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-3 text-sm rounded-lg ${message.type === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-gray-200 text-gray-800 rounded-tl-none'}`}>
                    {message.content}
                  </div>
                </div>)}
            </div>
